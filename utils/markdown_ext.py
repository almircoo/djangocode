            ...
            if re.search(self.RE_FENCE_END, block):
                ...
                for key in self.icon_dict.keys():
                    if key in original_block:
                        ...
                        if title:
                            icon_elm.set(
                                "class", "fa fa-{} mr-1".format(self.icon_dict[key])
                            )
...
class CodeGroupExtension(Extension):
    def extendMarkdown(self, md):
        md.parser.blockprocessors.register(
            CodeGroupBlockProcessor(md.parser), "code-group", 176
        )
...
class CodeItemExtension(Extension):
    def extendMarkdown(self, md):
        md.parser.blockprocessors.register(
            CodeItemBlockProcessor(md.parser), "code-item", 175
        )
...
if __name__ == "__main__":
    ...
    from markdown.extensions.toc import TocExtension  # Extende los puntos de anclaje
